...
export const calculateSalary = (state: SalaryState) => {
  ...
  const PERFORMANCE_BONUS = state.totalShifts < 22 ? 0 : 200; 
  ...
  // Base Pay
  const basePay = state.totalShifts < 22 ? state.totalShifts * DAILY_RATE : BASE_SALARY;
  ...
  // Social Security: <= 5 shifts = 0; > 5 shifts = 420.68
  const deductionSocial = state.totalShifts <= 5 ? 0 : 420.68;
